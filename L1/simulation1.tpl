...
//MODIFIED BY YANG FOR WHITEFISH MSE SIMULATION PROJECT 08/12/1012
...
// ***************************************following derived directly from the input data
  ...
  //vector MSY(1,numPop)   
  //vector EY(1,numPop)  
  //vector MSYpar(1,numPop)   
  //vector EYpar(1,numPop)  
  ...
  3darray R_err(1,numSim,1,numYear,1,numPop)          //temp autocorrelated recruitment error
 ...
 LOC_CALCS
   ...
   //cout<<"recruStd:"<<recruStd<<endl;
   //cout<<"recruAcor:"<<recruAcor<<endl;
    ...
    // **************************************************************after first year &  before starting assessment year data simulation
      if(j > 1 && j<=assessYr){ 
        for(int n=1;n<=numPop;n++){    
          ...
          tmpPop(1)=recruAlpha(n)*SSB(i,j-1,n)*mfexp(-recruBeta(n)*SSB(i,j-1,n))*mfexp(R_err(i,j,n)); // has E(R)=alpha'*S*exp(-beta*S), alpha'=alpha*exp(var/2)
       ...
       //cout<< "Recruit_error: " << R_err(i,j)<< "  " <<endl; 
...
// ***********************************************
  ...
  if(j > assessYr){                        // use new F for year after assessment started
        ...
        //cout<<"real SSB"<<real_ssb<<endl;
        //cout<<"assess SSB"<<assess_ssb<<endl;
        //cout<<"mre cal"<<endl;
        ...
        if (model!=1){  // begin model!=1 :     pooled population model 2
        ...
        //cout<<"new F basd on TAC implement"<<endl;
      ...
      if(j == assessOK){ //start on assessYr, then every 3 yrs after 
       ...
       if(model==1){
         ...
         //cout<<"Rec Mean"<<RecPop<<endl;
          ...
          readin>>assess_N(k); readin>>catch_mre(i,k,j); 
          readin>>catch_mare(i,k,j); readin>>assess_ssb(k);
          //*******here to check
