...
DATA_SECTION
  ...
  int assessLastOK                                                              // assess0K-assessStepYr
  ...
  matrix abun(1,numPop,assessFAge,assessLAge)                                   // build the population abundance at start of the yr
  vector recr(1,numPop)                                                         // all estiamted year recruit
  matrix Sel_est(1,numPop,assessFAge,assessLAge)   
  3darray F_est(1,numPop,1,lastNumYrMort-1,assessFAge,assessLAge)
  3darray F_est_update(1,numPop,1,lastNumYrMort-1,assessFAge,assessLAge)
  vector TAC(1,numPop)
  matrix predict_N(1,numPop,assessFAge,assessLAge)
  vector realYield(1,numPop) 
  vector poolabun(assessFAge,assessLAge)                                        // build the population abundance at start of the yr
  number poolrecr                                                               // all estiamted year recruit
  vector poolSel_est(assessFAge,assessLAge)         
  number poolTAC           
  vector predict_poolN(assessFAge,assessLAge)    
  number poolrealYield
  3darray yieldTolarea(1,numSim,1,numYear,1,numPop)
  matrix yieldTol(1,numSim,1,numYear)  
  vector Ftac(assessFAge,assessLAge)
  vector Zvec(assessFAge,assessLAge)
  vector Svec(assessFAge,assessLAge) 
  vector tacAge(assessFAge,assessLAge)     
  ...
  matrix pooled_ssb_mare(1,numSim,1,numYear)     
  3darray ssb_mre(1,numSim,1,numPop,1,numYear)
  3darray ssb_mare(1,numSim,1,numPop,1,numYear)                               // total abundance *weight for 4 areas mare                                                                   
  ...
  vector updated_F(1,numPop)
  number poolupdated_F
...
PRELIMINARY_CALCS_SECTION 
  ...
  pooledpopBegYr.initialize();yieldTolarea.initialize();yieldTol.initialize();pooledTAC.initialize();
  ...
  pooledcatch_mre.initialize();pooledcatch_mare.initialize(); ssb_mre.initialize();ssb_mare.initialize();
  pooledindicator   =0.;
  pooledhess_ind    =0.;
  pooledcounter     =0.;
  pooledhess_count  =0.;
  ...
  for(int i=1;i<=numSim;i++){          // **simulation loop  i
   ...
   for(int j=1;j<=numYear;j++){       // **year loop j      
    ...
    if(j == 1){                      //************************************first year data simulation 
     ...
     for(int k=1;k<=numPop;k++){  // **area  loop k
      for(int m=1;m<=numAge;m++){// **age loop m
       ...
       //yieldTolarea(i,j,k)+=areaCatch(i,j,k,m)*weight(m);            
      }                                         // end age m            
    ...
    for(int k=1;k<=numPop;k++) {
     ...
     //cout<<"Pop "<<k<<" yield total:"<< yieldTolarea(i,j,k);  
     for(int h=assessFAge;h<=assessLAge;h++)  yieldTolarea(i,j,k) += areaCatch(i,j,k,h)*weight(h);   
     yieldTolarea(i,j,k)=yieldTolarea(i,j,k)* mfexp((stdObsCatch(k)*randn(rnd))-(0.5*square(stdObsCatch(k))));
     F(i,j,k)= fishMort(k); 
     yieldTol(i,j)+=yieldTolarea(i,j,k); 
    }
...
// ***********************************************
  ...
  if(j > assessYr){                        // use new F for year after assessment started
        ...
        //cout<<"real_mixAbundance="<<endl;
        //for(int n=1;n<=numPop;n++) cout<<tmpPopMove(n)(assessFAge,assessLAge)<<endl;       
                
        NewtonRaphsonForF(tmpPopMove); // get new F based on annualTAC and using the abundance at two year before                  
        cout<<"realupdated F: "<< newF <<  "  " <<endl;       
        for(int n=1;n<=numPop;n++){      
          for(int k=1;k<=numPop;k++){   
            for(int m=1;m<=numAge;m++){         
 ...
 //****************************************************
  ...
  //**********************assessment on each assessment OK year***********************
      ...
      if(j == assessOK){ //start on assessYr 21 , then every 3 yrs after(24,27,30....) 
       ifstream fexist("sim4assessment.std"); //check if std file exist or not, if true, then delete it  
       if(fexist) {
        fexist.close(); 
       ...
       if(model==1){
         ...
         //cout<<"Rec Mean"<<RecPop<<endl;
          ...
          for(int k=1;k<=numPop;k++){                           
          ofstream report("sim4assessment1.dat");   //output data file for assessment
          ...
          report<<endl<<"#==== fishing effort  (year)  ===="<<endl;      //output fishing effort
          if(useAllYr==1){
            for(int yr=1;yr<=j-1;yr++)  //use all previous yr data
         ...
         //read in TAC from assessment report file
          ...
          readin>>abun(k); readin>>recr(k);
          readin>>Sel_est(k);
          readin>>F_est(k);
          readin>>test;
          //*******here to check
          /*
          cout<<"sim="<<i<<" yr="<<j<<" area="<<k<<"readInTAC(k)"<<readInTAC(k)<<"grad"<<grad<<endl;
          cout<<"assess_N(i,k,j)"<<assess_N(i,k,j)<<endl;
          cout<<"assess_ssb(i,k,j)"<<assess_ssb(i,k,j)<<endl;
          cout<<"catch_mre(i,k,j)"<<catch_mre(i,k,j)<<endl;
          cout<<"catch_mare(i,k,j)"<<catch_mare(i,k,j)<<endl;
          cout<<"abun(k)"<<abun(k)<<endl;
          cout<<"recr(k)"<<recr(k)<<endl;
          cout<<"Sel_est(k)"<<Sel_est(k)<<endl;
          cout<<"F_est(k)"<<F_est(k)<<endl;
          */
          readin.close();
          //cout<<"here we test data readin from assessment model"<<endl;
          //cout<<"F_est(k)"<<F_est(k)<<endl;

          //cout<<"assess_N(k)"<<assess_N(k)<<endl;
          //cout<<"assess_ssb(k)"<<assess_ssb(k)<<endl;
          //cout<<test<<endl;
          //exit(7);
       ...
       //********************pooled population assessment model begins here*****************************
         if( model==2){     
          ...
          readin>>readInTACpooled;                      
          readin>>pooledgrad;
          readin>>pooled_assess_ssb; readin>>pooledcatch_mre(i, j); readin>>pooledcatch_mare(i, j);
          readin>>poolabun; readin>>poolrecr;
          readin>>poolSel_est;
          readin.close();
        ...
        //cout<<"assessLastOK="<<assessLastOK<<" assessOK="<<assessOK<<endl;
        //exit(5);      
     }
     //************************end of each time assessment****************************** 
               

    //For the year without full assessment, we first calculate F for the lag year in the lastest full assessment based on additional yield data: FUNCTION:  realyieldupdate
    //then use that F with previous F estimation from full assessment model, to calculate the average F. That F occured in the current lag year
    //in FUNCTION: Fupdate_TAC. First do one year forward prediction for the 1-year lag. By the end of the lag year, fish survival became one year older in the year after the lag year-that is the year we predict TAC by assuming target mortality occurred
    //To repeat this process for all non-assessment year, information need to update in the process of "simple" assessment
    //information includes: readInTAC (which TAC used to update F, for the function NewtonRaphsonForF), abun (adundance at the begining of the last lag year in function Fupdate_TAC ), F_est( previous F estimation, should always include the last updated_F)
    if((j>assessYr)&&(j>assessLastOK) && (j<assessOK)){
    //cout<<"No full assessment: sim="<<i<<" yr="<<j<<endl;
    //exit(4);   
     if(model==1){      
      for(int k=1;k<=numPop;k++) realYield(k)=yieldTolarea(i,j-1,k);
      //cout<<"estmated_Abundance(from Last year)="<<endl;
      //cout<<abun<<endl;
      realyieldupdate_F(abun);      
      cout<<"yieldupdated_F="<<updated_F<<endl; //although cout in this loop, this F should be compariable with Year j-1 realupdated F
      //cout<<"Sel_real="<<select<<endl;
      //cout<<"Sel_est="<<Sel_est<<endl;
      Fupdate_TAC(abun);
      //cout<<"nextyear_TAC="<<TAC<<endl;
      //cout<<"predict_N="<<predict_N<<endl;
      for(int k=1;k<=numPop;k++) {
      readInTAC(k)= TAC(k);
      abun(k)=predict_N(k);
      F_est(k)=F_est_update(k);
      }
     }
     if(model==2){
      poolrealYield= yieldTol(i,j-1);
      poolrealyieldupdate_F(poolabun);
      cout<<"yieldupdated_F="<<poolupdated_F<<endl;
      poolFupdate_TAC(poolabun);
      readInTACpooled=poolTAC;
      poolabun=predict_poolN;
     }
    } 
   } //end year loop j
  }//end sim loop i
...
FUNCTION int realyieldupdate_F(const dmatrix&stockN)
  double criteria=0.0001;
  int maxLoop=1000;   
   for(int k=1;k<=numPop;k++){ //for each area
    double Fnew=0.1; //starting value, initial as .1
    double holder=0;  //hold Fnew value from last run
    double delta,tmpYield,tmpYieldH,tmpYieldL;    
    for(int l=1;l<=maxLoop;l++){  //fixed the loop number as 10
      delta=Fnew*0.001; //try make the step small
      tmpYield=0; tmpYieldH=0;tmpYieldL=0;
      for(int m=assessFAge;m<=assessLAge;m++){ //loop for all ages     age begin from age 3
        tmpYield+=weight(m)*(Sel_est(k,m)*Fnew/(natMort(k)+Sel_est(k,m)*Fnew))*stockN(k,m)*(1.-mfexp(-(natMort(k)+Sel_est(k,m)*Fnew)));//use Fnew, fill in catch equation
        tmpYieldH+=weight(m)*(Sel_est(k,m)*(Fnew+delta)/(natMort(k)+Sel_est(k,m)*(Fnew+delta)))*stockN(k,m)*(1.-mfexp(-(natMort(k)+Sel_est(k,m)*(Fnew+delta))));//use Fnew+delta
        tmpYieldL+=weight(m)*(Sel_est(k,m)*(Fnew-delta)/(natMort(k)+Sel_est(k,m)*(Fnew-delta)))*stockN(k,m)*(1.-mfexp(-(natMort(k)+Sel_est(k,m)*(Fnew-delta))));//use Fnew-delta
      }
      //following Fratio=f(x0)/f'(x0)
      double Fratio=(tmpYield-realYield(k))/((tmpYieldH-tmpYieldL)/(2.*delta));
      Fnew=Fnew-Fratio;  //update Fnew                
      if(Fnew<0) Fnew=0.5*(Fnew+Fratio);  //adjust it
      if(Fnew>3) Fnew=3;//At low N, Fnew would sometimes be really high, so limit here
      if(fabs(Fnew-holder)<criteria) break; //if meeting the convergence criteria, then exit
    ...
    //cout<<"Pop"<<k<<endl;
    //cout<<"esti_Yield: "<<tmpYield<<endl;
    //cout<<"real_Yield: "<<realYield(k)<<endl;
    //cout<<"esti_Sel"<<endl;
   // cout<<Sel_est(k)(assessFAge,assessLAge)<<endl;
    //cout<<"real_Sel"<<endl;
    //cout<<select(assessFAge,assessLAge)<<endl; 
    //cout<<"weight_at_age"<<endl;
    //cout<<weight(assessFAge,assessLAge)<<endl;
    //cout<<"real_F: "<<newF(k)<<endl;
    //cout<<"esti_F: "<<updated_F(k)<<endl;
   }
   return 0;

FUNCTION int poolrealyieldupdate_F(const dvector&stockN)
  double criteria=0.0001;
  int maxLoop=100;   
  double Fnew=0.1; //starting value, initial as .1
  double holder=0;  //hold Fnew value from last run
  ...
  for(int l=1;l<=maxLoop;l++){  //fixed the loop number as 10
      ...
      tmpYield=0; tmpYieldH=0;tmpYieldL=0;
      for(int m=assessFAge;m<=assessLAge;m++){ //loop for all ages     age begin from age 3
        tmpYield+=weight(m)*(poolSel_est(m)*Fnew/(natMort(1)+poolSel_est(m)*Fnew))*stockN(m)*(1.-mfexp(-(natMort(1)+poolSel_est(m)*Fnew)));//use Fnew, fill in catch equation
        tmpYieldH+=weight(m)*(poolSel_est(m)*(Fnew+delta)/(natMort(1)+poolSel_est(m)*(Fnew+delta)))*stockN(m)*(1.-mfexp(-(natMort(1)+poolSel_est(m)*(Fnew+delta))));//use Fnew+delta
        tmpYieldL+=weight(m)*(poolSel_est(m)*(Fnew-delta)/(natMort(1)+poolSel_est(m)*(Fnew-delta)))*stockN(m)*(1.-mfexp(-(natMort(1)+poolSel_est(m)*(Fnew-delta))));//use Fnew-delta
      }
      //following Fratio=f(x0)/f'(x0)
      double Fratio=(tmpYield-poolrealYield)/((tmpYieldH-tmpYieldL)/(2.*delta));
      Fnew=Fnew-Fratio;  //update Fnew                
      if(Fnew<0) Fnew=0.5*(Fnew+Fratio);  //adjust it
      if(Fnew>3) Fnew=3;//At low N, Fnew would sometimes be really high, so limit here
      if(fabs(Fnew-holder)<criteria) break; //if meeting the convergence criteria, then exit
...
FUNCTION int Fupdate_TAC(const dmatrix&abun) 
   //use last-year Z as the Z for year last+1  ,use average recruitment
    for (int k=1;k<=numPop;k++){ 
     dvar_matrix predict_abun(1,2,assessFAge,assessLAge); //predict the abundance for year j(current year) and one year forward (begging of that year before fishing), a temperol variable to save for all populations 
   //for year j+1 (beginning of yearj+1, before fishing: the full assessment year)
   //according to updated F of year j, which based on observed yield of Year j, and abundance estimation of the begining of year 21 
     for(int m=assessFAge+1;m<=assessLAge; m++) predict_abun(1,m) =abun(k,m-1)*mfexp(-(updated_F(k)*Sel_est(k,m-1)+natMort(k)));
     predict_abun(1,assessFAge)=recr(k);     
     predict_abun(1,assessLAge)+=abun(k,assessLAge)*mfexp(-(updated_F(k)*Sel_est(k,assessLAge)+natMort(k)));
    
   //for year j+2 (beginning of that year before fishing, one year after full assessment year, also the year we are interested in to set TAC)
   //One year lag occurred in year j+1,then fish just become one year's older at the begining of year j+2
   //Required F_average for the lag year
   //begining of year j+2, before fishing: according to what happened in the lag year j+1
     dmatrix F_tmp(1,lastNumYrMort,assessFAge,assessLAge);
     dvector average_F(assessFAge,assessLAge);
     
     for (int y=1;y<lastNumYrMort; y++) F_tmp(y)= row(F_est(k),y);
     for(int m=assessFAge;m<=assessLAge; m++) {
      F_tmp(lastNumYrMort,m)=updated_F(k)*Sel_est(k,m);
      average_F(m)=mean(column(F_tmp,m));
     }
     for(int m=assessFAge+1;m<=assessLAge; m++) predict_abun(2,m) =predict_abun(1,m-1)*mfexp(-(average_F(m-1)+natMort(k)));
     predict_abun(2,assessFAge)=recr(k);     
     predict_abun(2,assessLAge)+=predict_abun(1,assessLAge)*mfexp(-(average_F(assessLAge)+natMort(k)));  
     
    for(int y=1;y<lastNumYrMort; y++) row(F_est_update(k),y)=F_tmp(y+1);

  //TAC for year j+2
     for(int m=assessFAge;m<=assessLAge;m++){
      predict_N(k,m)=value(predict_abun(1,m)); //saved for updating abun in this function, abun is always the adundance estimation/prediction of the last lag year for the next state, which we applied F-average for in this state
      Ftac(m)=(fishMort(k))*Sel_est(k,m);  //use target total mortality and last yr selectivity
      Zvec(m)=natMort(k)+Ftac(m);
      Svec(m)=mfexp(-1.0*Zvec(m));   
      tacAge(m)=(Ftac(m)/Zvec(m))*(1-Svec(m))*predict_N(k,m);
     }
     TAC(k)=sum(tacAge);
    }   
   return 0;  
  ...
  /*
   //for the year j (end of current year after fishing: not assessment year)
     for(int m=assessFAge+1;m<=assessLAge; m++) predict_abun(1,m) =abun(k,m)*mfexp(-(updated_F(k)*Sel_est(k,m)+natMort(k)));
     
     //for the year j+1(end of year j)
     predict_abun(2,assessFAge)=recr(k);
     for(int m=assessFAge+1;m<=assessLAge;m++) predict_abun(2,m) =predict_abun(1,m-1)*mfexp(-(fishMort(k)*Sel_est(k,m)+natMort(k)));
     predict_abun(2,assessLAge)+=predict_abun(1,assessLAge)*mfexp(-(fishMort(k)*Sel_est(k,assessLAge)+natMort(k))); //last age group is a plus age group
   
     //for the year j+1(begining of year j)
     dvector tmpPop(assessFAge,assessLAge);
     tmpPop(assessFAge)=recr(k);
     for(int m=assessFAge+1;m<=assessLAge;m++) tmpPop(m)=value(predict_abun(1,m-1));
     tmpPop(assessLAge)+=value(predict_abun(1,assessLAge));

     //save the predicted abundance of year 2(begining) for later replacement of abun(k) 
     for(int m=assessFAge;m<=assessLAge;m++) predict_N(k,m)= tmpPop(m);
     //predict TAC for year j+1

     for(int m=assessFAge;m<=assessLAge;m++){
      Ftac(m)=(fishMort(k))*Sel_est(k,m);  //use target total mortality and last yr selectivity
      Zvec(m)=natMort(k)+Ftac(m);
      Svec(m)=mfexp(-1.0*Zvec(m));   
      //tac for year j+1 (year2 in loop)
      tacAge(m)=(Ftac(m)/Zvec(m))*(1-Svec(m))*tmpPop(m);
     }
     //elem_prod(elem_div(Ftac,Zvec),elem_prod(1.0-Svec,predict_abun(2)));  
     TAC(k)=sum(tacAge);
     }   
   return 0;    
   */
...
FUNCTION int poolFupdate_TAC(const dvector&poolabun)     
     dvar_matrix predict_poolabun(1,2,assessFAge,assessLAge); //predict the abundance for year j(current year) and one year forward  
     //for the year j (current year: not assessment year)
     for(int m=assessFAge+1;m<=assessLAge; m++) predict_poolabun(1,m)=poolabun(m)*mfexp(-(poolupdated_F*poolSel_est(m)+natMort(1))); 
     //for the year j+1
     for(int m=assessFAge+1;m<=assessLAge;m++) predict_poolabun(2,m) =predict_poolabun(1,m-1)*mfexp(-(fishMort(1)*poolSel_est(m)+natMort(1)));
     predict_poolabun(2,assessLAge)+=predict_poolabun(1,assessLAge)*mfexp(-(fishMort(1)*poolSel_est(assessLAge)+natMort(1))); //last age group is a plus age group
     predict_poolabun(2,assessFAge)=poolrecr;
   
   
     //for the year j+1(begining of year j)
     dvector tmpPop(assessFAge,assessLAge);
     tmpPop(assessFAge)=poolrecr;
     for(int m=assessFAge+1;m<=assessLAge;m++) tmpPop(m)=value(predict_poolabun(1,m-1));
     tmpPop(assessLAge)+=value(predict_poolabun(1,assessLAge));

     //save the predicted abundance of year 2(begining) for later replacement of abun(k) 
     for(int m=assessFAge;m<=assessLAge;m++) predict_poolN(m)= tmpPop(m);
     //predict TAC for year j+1

     //predict TAC for year j+1
     //dvector Ftac(assessFAge,assessLAge);
     //dvector Zvec(assessFAge,assessLAge);
     //dvector Svec(assessFAge,assessLAge); 
     //dvector tacAge(assessFAge,assessLAge);


     for(int m=assessFAge;m<=assessLAge;m++){
      Ftac(m)=(fishMort(1))*poolSel_est(m);  //use target total mortality and last yr selectivity
      Zvec(m)=natMort(1)+Ftac(m);
      Svec(m)=mfexp(-1.0*Zvec(m));   
      //tac for year j+1 (year2 in loop)
      tacAge(m)=(Ftac(m)/Zvec(m))*(1-Svec(m))*tmpPop(m);
     }
