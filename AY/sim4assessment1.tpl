...
PARAMETER_SECTION
  ...
  //report results
  number Recout
  //vector Mout(fage,lage)
  //vector Zout(fage,lage)
  vector Nout(fage,lage)
  matrix abund(lyear,lyear+2,fage,lage) //only used for tac calculation 
  matrix Fout(lyear-lastNumYrMort+2,lyear,fage,lage)
...
FUNCTION get_TAC 
  Recout= sum(column(n,fage)(lyear-lastNumYrRec,lyear-1) )/double(lastNumYrRec); //if not believe last year abundance estimation!!use 10 year averge till lyear -1
  
  abund(lyear)=n(lyear); //fill in the last yr abundance from n
  //change the first age to Recout if not believe lyear recru
  abund(lyear,fage)=Recout;
  
  
  //use last-year Z as the Z for year last+1  ,use average recruitment
  for(j=fage+1; j<=lage; j++) abund(lyear+1,j) =abund(lyear,j-1)*S(lyear,j-1);
  abund(lyear+1,lage)+=abund(lyear,lage)*S(lyear,lage); //last age group is a plus age group
  //abund(lyear+1,fage)=sum(column(n,fage)(lyear-lastNumYrRec+1,lyear) )/double(lastNumYrRec);    //use average recruitment
  //change the first age to Recout if not believe lyear recru
  abund(lyear+1,fage)=Recout;
  ...
  //for output report
  //Recout= sum(column(n,fage)(lyear-lastNumYrRec+1,lyear) )/double(lastNumYrRec);    //the last 10 yrs first age as mean recruitment
  //Recout= sum(column(n,fage)(lyear-lastNumYrRec,lyear-1) )/double(lastNumYrRec); //if not believe last year abundance estimation!!use 10 year averge till lyear -1
  Nout=abund(lyear+1);
  //change the first age to Recout if not believe lyear recru
  Nout(fage)= Recout;
//return num of fish at same age 
  for(int y=lyear-lastNumYrMort+2;y<=lyear;y++) Fout(y)=F(y);
...
REPORT_SECTION
  ...
  report<<Nout<<endl; //required for F estimation in "simple" assessment in the simulation model which based on addtional observed yield data (F for the current lag year) 
  report<<Recout<<endl; //required for adundance estimation non-full assessment year
  report<<rc_s(lyear)<<endl; //selectivity estimation for  F estimation in "simple" assessment
  report<<Fout<<endl;  //required for the 1-year lag for the "simple" assessment in simulation model based on observed yield data 
  report<<test<<endl;
  //report<< S(lyear)<<endl;
  //report<< n(lyear-1)<<endl;
  //report<< n(lyear)<<endl;
