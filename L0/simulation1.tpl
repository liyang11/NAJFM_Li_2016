...
DATA_SECTION
  ...
  init_int model                            // model1,2,3,4 represent four assessment methods, in Freq paper, only use model=1
  init_int mixLev                           // mix lev 1=no mixing; 2=half stay
  init_int TarALev                          // target total mortality level
  init_int RecVLev                          //target recruitment variation level
  init_int forLinux                         // 0 for windows, 1 for linux(mac and HPCC)
  ...
  vector recruAlpha_Pri(1,numPop)           // logistic recruit parameter,biased Alpha
  ...
  number effSampleSize                      // effective sample size
 ...
 LOC_CALCS
   ...
   recruAlpha_Pri = column(recruitPar,1); // logistic recruit parameter
...
PRELIMINARY_CALCS_SECTION 
  ...
  assess_N.initialize(); N_mre.initialize(); N_mare.initialize();mixBegYr.initialize();
  assess_ssb.initialize();
    ...
    // **************************************************************after first year &  before starting assessment year data simulation
      if(j > 1 && j<=assessYr){ 
        for(int n=1;n<=numPop;n++){    
          ...
          tmpPop(1)=recruAlpha(n)*SSB(i,j-1,n)*mfexp(-recruBeta(n)*SSB(i,j-1,n))*mfexp(R_err(i,j,n));
...
// ***********************************************
  ...
  if(j>assessYr){                        // use new F for year after assessment started
        ...
        int j_correct=j-1;            //we regard the mre of SSB estimation of year j as the assessment performance of year j-1, when we actually do assessment, so in final results, MRE_ssb for year 100=0
        for(int n=1;n<=numPop;n++){ 
          R_err(i,j,n)= recruStd*randn(rnd)+ recruAcor*R_err(i,j-1,n);
          tmpPop(n,1)=recruAlpha(n)*SSB(i,j-1,n)*mfexp(-recruBeta(n)*SSB(i,j-1,n))*mfexp(R_err(i,j,n));
...
// model=4 :: pooled population assessment model (ey)
  ...
  //*****assessmentOK ::assessOK+=assessStepYr  Start from assessYr 21
      ...
      if(j == assessOK){ //start on assessYr, then every 3 yrs after 
       ...
       if(model==1){
          ...
          for(int k=1;k<=numPop;k++){      
            ...
            int startYr=j-nYrAssess+1;
            IntPop(k)=mean(popBegYr(i,k,startYr)(assessFAge+1,assessLAge));
            for(int yr=startYr;yr<=j;yr++){
              dvector TmpPop(startYr,j);
              TmpPop(yr)=popBegYr(i,k,yr,assessFAge);
              //cout<<popBegYr(i,k,yr)<<endl;
              if(yr==j) RecPop(k)=mean(TmpPop);
              }
            }                                
           }         
          ...
          report<<"# sim,yr,area = "<<i<<", "<<j<<", "<<k<<endl; //atucally yr data from
          if(nYrAssess>assessYr) nYrAssess=assessYr;           //in case user input big number than actual assessment starting yr
          report<<"# num years, first Age, last age "<<endl;
          int tmpYr=(useAllYr==1)? j:nYrAssess;
          ...
          if(useAllYr==1){
            for(int yr=1;yr<=j;yr++)  //use all previous yr data
          ...
          readin>>test;
          //*******here to check
          //cout<<"sim="<<i<<" yr="<<j<<" pop="<<k<<"Fnew"<<newF(k)<<" tac="<<readInTAC(k) <<" mre="<<catch_mre(i,k,j)<<" mare="<<catch_mare(i,k,j)<<"grad"<<grad<<"test"<<test<<endl;
          readin.close();
          //exit(7);
       ...
       //********************pooled population assessment model begins here*****************************
         if( model==2){     
          ...
          report<<"# sim,yr = "<<i<<", "<<j<<endl; //atucally yr data from
          if(nYrAssess>assessYr-1) nYrAssess=assessYr;  //in case user input big number than actual assessment starting yr
          report<<"# num years, first Age, last age "<<endl;
          int tmpYr=(useAllYr==1)? j:nYrAssess;
          report<<tmpYr <<" "<<assessFAge<<" "<<assessLAge<<endl;
          report<<endl<<"# natural mort, total target mort "<<endl;
         ...
         for(int k=1;k<=numPop;k++){
          if(fabs(pooledgrad)>0.001) {
            ...
            pooledcounter(i)+=1.0;
            if (j!=assessYr) readInTACpooled=pooledTAC(i,j);
          }
         }
          //if got hessian warning
...
//*********************************************************************************************
  ...
  cout<<"Total assessment time(sim*years)" <<totAssess<<endl;
