...
PARAMETER_SECTION
  ...
  init_bounded_number log_mean_recruit(8.,18.,1)           //Average recruitment over the years - log scale
  ...
  matrix abund(lyear,lyear+1,fage,lage)       //for TAC calculation, and results output
  number Total_ssb                            //for results output, in order to calculate MRE of SSB prediction/estimation for the year of TAC setting in simulation tpl file
...
FUNCTION get_TAC
  //dvar_matrix abund(lyear,lyear+1,fage,lage); //only used for tac calculation
  ...
  dvariable tmpMort; 
  //abundance at begining of year lyear+1
  ...
  //TAC calculation, this is the case without 1-year lag
  Ftac=(tot_targ_Z-nat_mort)*rc_s(lyear);  //use target total mortality and last yr selectivity
  Mvec=M(lyear);
  Zvec=Mvec+Ftac;
  Svec=mfexp(-1.0*Zvec);
  tacAge=elem_prod(elem_div(Ftac,Zvec),elem_prod(1.0-Svec,abund(lyear+1)));
  Total_ssb =sum(elem_prod(elem_prod(abund(lyear+1), wt_age),mat_age)*propFemale);
...
REPORT_SECTION
  ...
  report<<abund(lyear+1)<<endl;  //estimated abundance at age at the beginning of the TAC setting year, for calculating mre of N estimation 
  ...
  report<<Total_ssb<<endl;  //estimated total SSB at the beginning of the TAC setting year, for calculating mre of SSB estimation/prediction
  report<<test<<endl;
  /*
