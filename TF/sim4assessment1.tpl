...
PARAMETER_SECTION
  ...
  number Total_ssb                     //Predicted total abundance weight(~ssb)
  ...
  matrix abund(lyear,lyear+assessFreq+1,fage,lage); //only used for tac calculation

  number L1  // components of the objective function
  number L2
  number L3
  number rc_cps_spl
  matrix tacAge(lyear+2,lyear+assessFreq+1,fage,lage) //TAC at age
...
FUNCTION get_TAC
  ...
  abund(lyear)=n(lyear); //fill in the last yr abundance from n, abudance at begining of the year
  ...
  //1. Identify what happend to the year lyear+1(21) given the mortality occurred in lyear.
  //use last-year Z as the Z for year last+1  ,use average recruitment
  for(j=fage+1; j<=lage; j++) abund(lyear+1,j) =abund(lyear,j-1)*mfexp(-Z(lyear,j-1));  //abundance at beginning of year lyear+1
  ...
  //2. cal the adundance by the end of year lyear+1(21) given the mortality occurred in lyear+1. In codes shows like the begining of year lyear+2, and 1 years older
  //use average Z as the Z for year last+1  ,use average recruitment
  dvariable tmpMort; 
  for(j=fage+1; j<=lage; j++)
   {
   tmpMort=mean(column(Z,j-1)(lyear-lastNumYrMort+1,lyear) ); //the average Mort at age j-1 for the last lastNumYrMort yrs as mean mortality for TAC
   abund(lyear+2,j) =abund(lyear+1,j-1)*mfexp(-tmpMort); //those survival fish became one year older at the begining of year lyear+2, but still used to discribe the harvest in year lyear+1(the lag year)  
   }
  ...
  //if not believe lyear recru esti, use the same recru average from year lyear, lyear+1
  ...
  Zvec=Mvec+Ftac;
  Svec=mfexp(-1.0*Zvec);
  tacAge(lyear+2)=elem_prod(elem_div(Ftac,Zvec),elem_prod(1.0-Svec,abund(lyear+2)));
  Total_ssb = sum(elem_prod(abund(lyear+2),elem_prod(wt_age,mat_age))*propFemale);
             
  for(i=lyear+3; i<=lyear+assessFreq+1; i++){
   for(j=fage+1; j<=lage; j++) abund(i,j)=abund(i-1,j-1)*mfexp(-Zvec(j-1));
   abund(i,lage)+=abund(i-1,lage)*mfexp(-Zvec(lage));
   //abund(i,fage)=sum(column(n,fage)(lyear-lastNumYrRec+1,lyear) )/double(lastNumYrRec);
   abund(i,fage)=sum(column(n,fage)(lyear-lastNumYrRec,lyear-1) )/double(lastNumYrRec); 
   tacAge(i)=elem_prod(elem_div(Ftac,Zvec),elem_prod(1.0-Svec,abund(i)));
  }
...
REPORT_SECTION
  get_TAC();
  for(i=lyear+2;i<=lyear+assessFreq+1;i++) report<<sum(tacAge(i))<<endl;
  report<<objective_function_value::gmax <<endl;
  report<<abund(lyear+2)<<endl;  //estimated total abundance in the last year of the assessment 
  report<<Total_ssb<<endl; 
